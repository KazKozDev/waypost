...
WEIGHTS = {
    ...
    "batch": dict(quality=1.2, latency=0.05, burn=1.0, affinity=0.2, reliability=0.5),
...
class Router:
    ...
    def _passes(
        ...
        if self.free_only and not o.free:
            return False
        if not self.breaker.allows(o.provider):
            return False
    ...
    def _score(
        ...
        latency = min(1.0, o.ttft_p50_ms / 5000.0)
        ...
        quality = (
            self.bandit.quality(p.task_class, o.key, base_quality)
            if self.bandit
            else base_quality
        )
    ...
    def _auto_plan(
        ...
        *,
        ...
        cloud_cands = [c for c in cands if not c.offering.is_local]
